...
impl Default for ImmixMutatorGlobal {
    fn default() -> Self {
        Self::new()
    }
}
...
impl fmt::Display for ImmixMutatorLocal {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.cursor.is_zero() {
            ...
            writeln!(f, "Mutator:").unwrap();
            writeln!(f, "cursor= {:#X}", self.cursor).unwrap();
            writeln!(f, "limit = {:#X}", self.limit).unwrap();
            writeln!(f, "line  = {}", self.line).unwrap();
