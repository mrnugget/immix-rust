...
impl Bitmap {
    pub fn new(length: usize) -> Bitmap {
        ...
        let bitmap = unsafe {
            ...
            libc::malloc((mem::size_of::<u64>() * ((bitmap_len >> 6) + 1)) as libc::size_t)
                as *mut u64
        };

        Bitmap { bitmap, bitmap_len }
    }

    #[inline(always)]
    pub fn set_bit(&mut self, index: usize) {
        let word = unsafe { self.bitmap.add(index >> 6) };
        unsafe { *word |= 1 << (index & 63) };
    }

    #[inline(always)]
    pub fn clear_bit(&mut self, index: usize) {
        let word = unsafe { self.bitmap.add(index >> 6) };
        unsafe { *word &= 0 << (index & 63) };
    }
    ...
    pub fn set(&mut self, index: usize, value: u64, length: usize) {
        ...
        let word = unsafe { self.bitmap.add(nth_u64) };
        ...
        if length <= 64 - nth_bit {
            ...
            unsafe {
                ...
                *word |= value.wrapping_shl(nth_bit as u32);
                *next_word |= value >> (64 - nth_bit);
            }
        }
    }
